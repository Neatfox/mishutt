...
public class ImageSliderAdAdapter extends SliderViewAdapter<ImageSliderAdAdapter.ViewHolder> {

    private final Context context;
    ArrayList<ImageSlider> image_list;

    public ImageSliderAdAdapter(Context context,  ArrayList<ImageSlider> image_list) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.adapter_image_slider_ad,viewGroup,false);
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String _string = imageSlider.getUrl();
                if (_string.length()>4){
                    if (imageSlider.getUrl().contains("http")){
                        Uri uri = Uri.parse(_string); // missing 'http://' will cause crashed
                        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
                        context.startActivity(intent);
                    } else {
                        System.out.println("Add http:// in URL");
                    }
                } else {
